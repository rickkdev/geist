...
class SecureLoggingFilter(logging.Filter):
    ...
    SENSITIVE_PATTERNS = [
        ...
        (r'\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b', '[UUID_SCRUBBED]'),
        
        # File paths that might contain sensitive info
        (r'/[a-zA-Z0-9._/-]*(?:key|cert|secret|private)[a-zA-Z0-9._/-]*', '[PATH_SCRUBBED]'),
    ]
...
class SecureLoggingMiddleware(BaseHTTPMiddleware):
    ...
    def _log_response(self, request: Request, response: Response, 
                     start_time: datetime, request_id: str):
        """
...
def secure_operation_context(operation_name: str, request_id: Optional[str] = None):
    ...
    try:
        ...
        duration = (datetime.utcnow() - start_time).total_seconds()
        logger.debug(f"Completed secure operation: {operation_name} [{request_id}] in {duration:.3f}s")
        
    except Exception as e:
        duration = (datetime.utcnow() - start_time).total_seconds()
        logger.error(f"Failed secure operation: {operation_name} [{request_id}] - {type(e).__name__} after {duration:.3f}s")
        raise
    ...
    if isinstance(data, dict):
        ...
        for key, value in data.items():
            ...
            if any(sensitive in key.lower() for sensitive in 
                   ['password', 'key', 'secret', 'token', 'auth', 'cipher', 'content', 'message']):
        ...
        if len(data) > 50 and all(c in 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=' for c in data):
