...
class TestRunner:
    ...
    def check_dependencies(self) -> bool:
        ...
        for name, cmd in dependencies:
            try:
                result = subprocess.run(cmd.split(), capture_output=True, text=True, cwd=self.project_root)
    ...
    def run_all_tests(self, quick: bool = False, verbose: bool = False) -> bool:
        ...
        if not quick:
            test_stages.extend([
                ("Integration Tests", lambda: self.run_integration_tests(verbose=verbose)),
                ("Security Tests", lambda: self.run_security_tests(verbose=verbose)),
            ])
...
Examples:
  ...
  python tests/run_tests.py quick                  # Run quick test suite (linting + unit)
    ...
    parser.add_argument(
        "command",
        choices=[
            "all", "unit", "integration", "security", "load", 
            "lint", "type", "coverage", "benchmark", "quick"
        ],
    ...
    parser.add_argument(
        "--duration", "-d",
        default="2m",
        help="Load test duration (default: 2m)"
    )
