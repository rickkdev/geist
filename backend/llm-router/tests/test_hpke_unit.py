...
class TestHPKEService:
    ...
    def test_decrypt_request_expired_timestamp(self, hpke_service):
        ...
        old_timestamp = datetime.now(timezone.utc) - timedelta(seconds=120)  # 2 minutes old
    ...
    def test_decrypt_request_future_timestamp(self, hpke_service):
        ...
        future_timestamp = datetime.now(timezone.utc) + timedelta(seconds=60)  # 1 minute future
    ...
    def test_encrypt_chunk(self, hpke_service):
        ...
        encrypted_chunk = hpke_service.encrypt_chunk(test_chunk, recipient_pubkey, sequence=0)
    ...
    def test_encrypt_chunk_multiple_sequences(self, hpke_service):
        ...
        for seq in range(5):
            encrypted_chunk = hpke_service.encrypt_chunk(f"Chunk {seq}", recipient_pubkey, sequence=seq)
            chunk_data = json.loads(encrypted_chunk)
