...
class InferenceService:
    ...
    async def stream_inference(self, request: InferenceRequest) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            logging.debug(f"Request params: temp={request.temperature}, top_p={request.top_p}, max_tokens={request.max_tokens}")
            ...
            async with self.client.stream(
                ...
                async for line in response.aiter_lines():
                    ...
                    if current_time > request_deadline:
                        logging.warning(f"Request {request.request_id} exceeded budget timeout of {budget_timeout}s")
                    ...
                    if line.startswith("data: "):
                        ...
                        if data.strip() == "[DONE]":
                            logging.info(f"Stream completed for request {request.request_id}, tokens: {token_count}")
                            break
                            ...
                            if "choices" in chunk_data and chunk_data["choices"]:
                                ...
                                if "content" in delta and delta["content"] is not None:
                                    ...
                                    if token:  # Only yield non-empty tokens
                                        token_count += 1
                                        logging.debug(f"Yielding token {token_count}: {repr(token[:50])}")
                                        yield token
                                        
                                # Check if this is the final chunk
                                finish_reason = choice.get("finish_reason")
                                if finish_reason:
                                    logging.info(f"Stream finished for request {request.request_id}, reason: {finish_reason}")
                                    break
                                    
                        except json.JSONDecodeError as e:
                            logging.warning(f"Failed to parse SSE data: {e}, data: {data[:100]}")
                            continue
