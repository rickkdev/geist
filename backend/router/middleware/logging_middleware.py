...
class RequestLoggingFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if (
            self.settings.DISABLE_REQUEST_BODY_LOGGING
            and hasattr(record, "pathname")
            and "request" in record.getMessage().lower()
        ):
            return False

        # Skip response body logging if disabled
        if (
            self.settings.DISABLE_RESPONSE_BODY_LOGGING
            and hasattr(record, "pathname")
            and "response" in record.getMessage().lower()
        ):
            return False
...
def log_security_event(
    event_type: str, details: Dict[str, Any], level: int = logging.WARNING
):
    """
