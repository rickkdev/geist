...
class HPKEService:
    ...
    def __init__(self, settings: Settings):
        ...
        self.key_id = "key-001"
        self.key_expires_at = datetime.now(timezone.utc) + timedelta(
            hours=settings.HPKE_KEY_ROTATION_HOURS
        )
    ...
    def _initialize_keys(self):
        ...
        try:
            ...
            if os.path.exists(
                self.settings.ROUTER_HPKE_PRIVATE_KEY_PATH
            ) and os.path.exists(self.settings.ROUTER_HPKE_PUBLIC_KEY_PATH):
    ...
    def decrypt_request(self, request: ChatRequest) -> DecryptedChatPayload:
        ...
        try:
            ...
            )  # encapsulated_key used for HPKE context
            ciphertext = base64.b64decode(request.ciphertext)
            _ = base64.b64decode(
                request.aad
            )  # aad used for additional authentication data
    ...
    def encrypt_chunk(
        self, chunk_data: str, recipient_public_key: bytes, sequence: int = 0
    ) -> str:
        """
        ...
        try:
            ...
            simulated_enckey = base64.b64encode(b"mock_enckey_for_chunk").decode(
                "ascii"
            )
    ...
    def get_public_keys(self) -> Dict[str, Any]:
        ...
        """
        current_pubkey_b64 = (
            base64.b64encode(self.current_public_key).decode("ascii")
            if self.current_public_key
            else ""
        )
        next_pubkey_b64 = (
            base64.b64encode(self.next_public_key).decode("ascii")
            if self.next_public_key
            else None
        )
    ...
    def _check_replay_protection(self, request: ChatRequest) -> bool:
        ...
        self.seen_request_ids = {
            rid: timestamp
            for rid, timestamp in self.seen_request_ids.items()
            if timestamp > cutoff
        }
    ...
    def rotate_keys(self):
        ...
        if self.next_private_key and self.next_public_key:
            ...
            self.key_expires_at = datetime.now(timezone.utc) + timedelta(
                hours=self.settings.HPKE_KEY_ROTATION_HOURS
            )
    ...
    def _generate_next_keys(self):
        ...
        self.next_public_key = self._public_key_to_bytes(
            self.next_private_key.public_key()
        )
    ...
    def _generate_new_key_pair(self):
        ...
        self.current_public_key = self._public_key_to_bytes(
            self.current_private_key.public_key()
        )
    ...
    def _load_keys_from_file(self):
        ...
        try:
            ...
            with open(self.settings.ROUTER_HPKE_PRIVATE_KEY_PATH, "rb") as f:
                ...
                self.current_private_key = serialization.load_pem_private_key(
    ...
    def _save_keys_to_file(self):
        ...
        try:
            ...
            os.makedirs(
                os.path.dirname(self.settings.ROUTER_HPKE_PRIVATE_KEY_PATH),
                exist_ok=True,
            )
