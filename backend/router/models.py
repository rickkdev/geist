...
class PubkeyResponse(BaseModel):
    ...
    next_pubkey: Optional[str] = Field(
        None, description="Next HPKE public key for rotation (base64)"
    )
...
class MetricsResponse(BaseModel):
    ...
    circuit_breaker_state: str = Field(
        ..., description="Circuit breaker state (open/closed/half-open)"
    )
    inference_latency_p50: float = Field(
        ..., description="50th percentile inference latency (ms)"
    )
    inference_latency_p95: float = Field(
        ..., description="95th percentile inference latency (ms)"
    )
    ...
    node_health_status: Dict[str, Any] = Field(
        ..., description="Detailed health status of all nodes"
    )
